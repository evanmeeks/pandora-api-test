					...
					fetch('https://www.pandora.com/api/v1/station/getFeedback', {
						...
						.then(function(response) {
							...
							response.json().then(function(data) {
								...
								document.writeln('<h1>Thumbs Up Tracks</h1>');
								data.feedback.forEach((thumb, k) => {
									var { albumTitle, songTitle, title, artistName } = thumb;
									console.log(k + ' Title: ', songTitle);
									console.log(k + ' Artist: ', artistName);
									document.writeln(k + ' Title: ' + songTitle + '<br/>');
									document.writeln(k + ' Artist: ' + artistName + '<br/>');
								});
							});
						})
						.catch(function(err) {
